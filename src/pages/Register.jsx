      ...
      <motion.div initial={{opacity:0, y:16, scale:0.98}} animate={{opacity:1, y:0, scale:1}} transition={{duration:0.6, ease:[0.22,1,0.36,1]}} className="relative w-full max-w-[680px] bg-white rounded-[24px] border border-slate-200 shadow-[0_20px_60px_rgba(15,23,42,0.10)] overflow-hidden">
        <div className="bg-slate-900 text-white px-6 py-5 flex items-center justify-between">
          <motion.div variants={container} initial="hidden" animate="visible" className="flex items-center gap-3">
            <motion.div variants={item} className="w-9 h-9 rounded-xl bg-teal-500 flex items-center justify-center"><Stethoscope size={18} /></motion.div>
            <div>
              <motion.div variants={item} className="font-bold leading-none flex items-center gap-2">Đăng ký bác sĩ <Sparkles size={14} className="text-teal-300" /></motion.div>
              <motion.div variants={item} className="text-xs text-slate-300">Da liễu • Private • RLS • Realtime</motion.div>
            </div>
          </motion.div>
        ...
        <motion.form variants={container} initial="hidden" animate="visible" onSubmit={submit} className="p-6 space-y-4">
          ...
          <div className="grid md:grid-cols-2 gap-3">
            <motion.label variants={item} className="block md:col-span-2">
              ...
              <input onFocus={()=>playClick('pop')} required value={form.full_name} onChange={e=>setForm({...form, full_name:e.target.value})} placeholder="BS. Nguyễn Văn A" className="mt-1.5 w-full px-3.5 py-2.5 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-teal-500/30 focus:border-teal-500 text-sm" />
            </motion.label>
            <motion.label variants={item} className="block">
              <span className="text-xs font-semibold tracking-widest text-slate-500">EMAIL *</span>
              <input onFocus={()=>playClick('pop')} type="email" required value={form.email} onChange={e=>setForm({...form, email:e.target.value})} placeholder="bacsi@benhvien.vn" className="mt-1.5 w-full px-3.5 py-2.5 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-teal-500/30 focus:border-teal-500 text-sm" />
            </motion.label>
            <motion.label variants={item} className="block">
              <span className="text-xs font-semibold tracking-widest text-slate-500">SĐT DI ĐỘNG</span>
              <input onFocus={()=>playClick('pop')} value={form.phone} onChange={e=>setForm({...form, phone:e.target.value})} placeholder="0901234567" className="mt-1.5 w-full px-3.5 py-2.5 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-teal-500/30 focus:border-teal-500 text-sm" />
            </motion.label>

            <motion.label variants={item} className="block">
              <span className="text-xs font-semibold tracking-widest text-slate-500">MẬT KHẨU *</span>
              <div className="relative mt-1.5">
                <input onFocus={()=>playClick('pop')} required value={form.password} onChange={e=>setForm({...form, password:e.target.value})} type={showPassword?'text':'password'} placeholder="≥6 ký tự" className="w-full px-3.5 py-2.5 pr-11 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-teal-500/30 focus:border-teal-500 text-sm" />
            ...
            <motion.label variants={item} className="block">
              ...
              <input onFocus={()=>playClick('pop')} value={form.specialty} onChange={e=>setForm({...form, specialty:e.target.value})} placeholder="Da liễu" className="mt-1.5 w-full px-3.5 py-2.5 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-teal-500/30 focus:border-teal-500 text-sm" />
            </motion.label>
            <motion.label variants={item} className="block">
              <span className="text-xs font-semibold tracking-widest text-slate-500">MÃ CCHN</span>
              <input onFocus={()=>playClick('pop')} value={form.license_code} onChange={e=>setForm({...form, license_code:e.target.value})} placeholder="012345/CCHN" className="mt-1.5 w-full px-3.5 py-2.5 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-teal-500/30 focus:border-teal-500 text-sm" />
            </motion.label>
            <motion.label variants={item} className="block md:col-span-2">
              <span className="text-xs font-semibold tracking-widest text-slate-500">VAI TRÒ</span>
              <select onFocus={()=>playClick('pop')} value={form.role} onChange={e=>{ playClick('tap'); setForm({...form, role:e.target.value})}} className="mt-1.5 w-full px-3.5 py-2.5 rounded-xl border border-slate-200 bg-white focus:outline-none focus:ring-2 focus:ring-teal-500/30 focus:border-teal-500 text-sm">
          ...
          <motion.div variants={item} className="rounded-xl bg-slate-50 border border-slate-200 px-3.5 py-3 text-xs text-slate-600 flex gap-2">
            ...
            <div>Mật khẩu mã hoá (Supabase Auth) • Lưu realtime vào <span className="font-mono">profiles</span> • RLS bảo vệ.</div>
          </motion.div>
          ...
          <motion.button variants={item} whileTap={{scale:0.98}} whileHover={{scale:1.01}} disabled={loading} className="w-full py-3 rounded-xl bg-teal-600 text-white text-sm font-semibold hover:bg-teal-700 disabled:opacity-50 inline-flex items-center justify-center gap-2">
            {loading?'Đang tạo…':<>Tạo tài khoản bác sĩ <ArrowRight size={16} /></>}
          </motion.button>
